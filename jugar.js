class Game {
    ...
    play() {
      ...
      if (allPlayers !== undefined) {
        ...
        for (var plr in allPlayers) {
          ...
          if(currentLife<=0){
            hero[index-1].changeImage("blast");
            hero[index-1].scale=0.3;
          }
  
        hero[index - 1].position.x = x;
          hero[index - 1].position.y = y;
          ...
          if (index === player.index) {
            ...
            camera.position.y = hero[index - 1].position.y;
          }
        }
    ...
    handleCarACollisionWithCarB(index){
      if(index==1){
        if (hero[index-1].collide(hero[1])){
      ...
      if(index==2){
        if (hero[index-1].collide(hero[0])){
          if(this.leftKeyActive){
            player.positionX+=100;
          }
          else {
    ...
    handleFuel(index) {
      // Agregar combustible
      hero[index - 1].overlap(fuels, function(collector, collected) {
        player.fuel = 185;
    ...
    handlePowerCoins(index) {
      hero[index - 1].overlap(powerCoins, function(collector, collected) {
        player.score += 21;
        player.update();
        //recolectado está el sprite en el grupo de recolectados que desencadenaron
        //el evento
    ...
    handleObstacleCollision(index) {
      if (hero[index - 1].collide(obstacles)) {
        if (this.leftKeyActive) {
          player.positionX += 100;
        } else {
          player.positionX -= 100;
